...
def KFold(x,y,z, k, model, poly_deg, test_B, lambda_ = 0.5): 
    ...
    split = np.array_split(data,k) # Splits the dataset into three folds
    length = len(split[0])
    ...
    for i in range(0, k): 
        ...
        #Designating one part for test and two parts to train
        ...
        #Preforming regression model on train dataset       

        xy_comp_train = XY(train[:,0],train[:,1], poly_deg) # creating design matrix with train data

        beta = Beta_model(xy_comp_train, train[:,2], model, lambda_, test_B) #obtaining beta values from model

        z_predict_val = KF_val.dot(beta) #prediction created from beta values derived from train dataset

        xy_comp_test = XY(test[:,0],test[:,1], poly_deg) #creating design matrix from test dataset

        z_predict_test = xy_comp_test.dot(beta) #Test dataset fitted using beta values for design matrix obtained from train dataset


        #Appending values obtained from functions

        mse.append(MSE(y_test, z_predict_val)) #MSE calculated using true data compared with modeled

        r2.append(R2(test[:,2], z_predict_test)) #R2 calculated using test dataset not used in the modeling, comparing it to the modeled values
